function cellBg(pnl, trades) {
  if (trades === 0) return 'var(--color-bg-card)';
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(160px, 1fr))', gap: 8 }}>
        {data.map(s => (
          <div key={s.sector} style={{
            ...
            {s.trades > 0 && (
              <div style={{ fontFamily: 'var(--color-text-mono)', fontSize: 13, fontWeight: 700, color: s.pnl >= 0 ? '#86efac' : '#fca5a5', marginBottom: 4 }}>
                {fmtPnl(s.pnl)}
              </div>
            )}
            ...
            <div style={{ fontSize: 10, color: 'var(--color-text-dim)', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
